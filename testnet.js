...
async function test(chains, wallet, options) {
    ...
    const source = chains.find((chain) => chain.name === (args[0] || "Polygon"));
    const destination = chains.find((chain) => chain.name === (args[1] || 'Avalanche'));
    const amount = Math.floor(parseFloat(args[2])) * 1e6 || 10e6;
    const accounts = ["0x294d985B6BC5dA375b571B5fDE228334343f4EdF"];
    const message = "first transaction";
    ...
    for (const chain of [source, destination]) {
        const provider = getDefaultProvider(chain.rpc);
        chain.wallet = wallet.connect(provider);
        chain.contract = new Contract(chain.contractAddress, DistributionExecutable.abi, chain.wallet);
        chain.gateway = new Contract(chain.gateway, Gateway.abi, chain.wallet);
        const usdcAddress = chain.gateway.tokenAddresses('aUSDC');
        chain.usdc = new Contract(usdcAddress, IERC20.abi, chain.wallet);
    }
    ...
    console.log("finished aprove txn");
    ...
    console.log("sent txn waiting for 2 mins...")
    await sendTx.wait(2);
